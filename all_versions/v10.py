...
app = FastAPI(
    ...
    description="Selenium-Driverless with JS click retries, cool downs, and adaptive sleeps.",
    version="1.5.5" # Incremented
)
...
INITIAL_PAGE_LOAD_SLEEP_S = random.uniform(0.5, 1.0) 
...
# BS Selectors (from v5.py) - No changes here
...
async def prepare_driverless_driver() -> driverless_webdriver.Chrome: # Unchanged
...
async def async_is_element_displayed_js(driver: driverless_webdriver.Chrome, element: driverless_webdriver.WebElement) -> bool: # Unchanged
...
async def async_is_element_present_in_dom_js(driver: driverless_webdriver.Chrome, element: driverless_webdriver.WebElement) -> bool: # Unchanged
    if not element: return False
    ...
    try:
        ...
        if always_js_click:
            for attempt in range(3): 
                try:
                    ...
                    return True 
                except asyncio.TimeoutError as e_timeout_js: 
                    print(f"    [{thread_name}][try_click_element] JS click attempt {attempt + 1} timed out.") # Simplified log
                    if attempt == 2: 
                        print(f"    [{thread_name}][try_click_element] JS click failed after multiple timeout attempts (original error: {e_timeout_js}).")
                        return False 
        ...
        for attempt in range(2): # JS Fallback click with retry
            try:
                ...
                await driver.execute_script("arguments[0].click();", element)
                return True
            except asyncio.TimeoutError as e_timeout_js_fb:
                print(f"    [{thread_name}][try_click_element] JS fallback click attempt {attempt + 1} timed out.")
                if attempt == 1: 
                    print(f"    [{thread_name}][try_click_element] JS fallback click failed after multiple timeout attempts (original error: {e_timeout_js_fb}).")
                    return False
...
async def handle_specific_overlay_popup(driver: driverless_webdriver.Chrome, thread_name: str) -> bool: # Unchanged
    try:
...
async def async_wait_until_reviews_load(driver: driverless_webdriver.Chrome, initial_review_count: int, review_card_selector: str, loader_selector: str, timeout: float, thread_name: str ) -> bool: # Unchanged
    start_time = time.monotonic()
    loader_was_seen_and_disappeared_without_increase = False
    while time.monotonic() - start_time < timeout:
        current_reviews_elements = []
        try:
            current_reviews_elements = await driver.find_elements(By.CSS_SELECTOR, review_card_selector, timeout=SELENIUM_ELEMENT_FIND_TIMEOUT_S / 4) 
...
async def _get_total_reviews_on_site_selenium(driver: driverless_webdriver.Chrome, thread_name: str) -> Optional[int]: # Unchanged from v1.5.4
    primary_selectors_with_pattern_check = ["span.e1xzmg0z.sr2r3oj", 'span[class*="sr2r3oj"]']
    try:
        ...
        for i, selector in enumerate(primary_selectors_with_pattern_check):
            ...
            try:
                candidate_spans = await driver.find_elements(By.CSS_SELECTOR, selector, timeout=SELENIUM_ELEMENT_FIND_TIMEOUT_S / (2 * len(primary_selectors_with_pattern_check)))
                if not candidate_spans and i == 0: print(f"      [{thread_name}] No elements found with selector '{selector}'."); continue
                for el_idx, el in enumerate(candidate_spans):
                    try:
                        text = await el.text
                        if not text: continue
                        match = re.search(r"([\d\.]+)\s*\((\d{1,3}(?:,\d{3})*|\d+)\)", text)
                        if match and match.group(2):
                            count_str = match.group(2).replace(",", ""); print(f"      [{thread_name}] Found matching pattern! Extracted total review count '{count_str}' from element {el_idx} (selector '{selector}') with text: '{text[:50]}...'"); return int(count_str)
                    except StaleElementReferenceException: print(f"      [{thread_name}] Stale element encountered for selector '{selector}', element index {el_idx}. Skipping.")
                    except Exception as e_el_text: print(f"      [{thread_name}] Error processing element {el_idx} for selector '{selector}': {type(e_el_text).__name__} - {e_el_text}. Skipping.")
            except TimeoutException: print(f"      [{thread_name}] Timeout for selector '{selector}'.")
        print(f"      [{thread_name}] All primary attempts with text pattern check failed to find total review count.")
        fallback_selector = 'span.typo-30.font-semibold'
        print(f"      [{thread_name}] Fallback attempt: Trying selector for review count display text: '{fallback_selector}'")
        try:
            count_display_candidates = await driver.find_elements(By.CSS_SELECTOR, fallback_selector, timeout=SELENIUM_ELEMENT_FIND_TIMEOUT_S / 2)
            for el in count_display_candidates:
                try:
                    text = await el.text
                    if not text: continue
                    match_of = re.search(r"of\s+([\d,]+)\s+Reviews", text, re.IGNORECASE)
                    if match_of: count_str = match_of.group(1).replace(",", ""); print(f"      [{thread_name}] Extracted total review count '{count_str}' from 'of X Reviews' text: '{text[:50]}...'"); return int(count_str)
                    match_total_only = re.search(r"^([\d,]+)\s+Reviews$", text.strip(), re.IGNORECASE)
                    if match_total_only: count_str = match_total_only.group(1).replace(",", ""); print(f"      [{thread_name}] Extracted total review count '{count_str}' from 'X Reviews' text: '{text[:50]}...'"); return int(count_str)
                except StaleElementReferenceException: print(f"      [{thread_name}] Stale element encountered for fallback selector '{fallback_selector}'. Skipping.")
                except Exception as e_fb_el_text: print(f"      [{thread_name}] Error processing element for fallback selector '{fallback_selector}': {type(e_fb_el_text).__name__} - {e_fb_el_text}. Skipping.")
        except TimeoutException: print(f"      [{thread_name}] Timeout for fallback selector '{fallback_selector}'.")
        print(f"      [{thread_name}] Total review count not found via any Selenium method (primary or fallback).")
        return None
    except Exception as e_general: print(f"      [{thread_name}] General error during Selenium total review count extraction: {type(e_general).__name__} - {e_general}"); traceback.print_exc(); return None

async def _load_all_capterra_reviews_driverless(product_url_str: str, company_slug: str) -> Tuple[Optional[str], str]:
    thread_name = f"CapterraLoad-{company_slug[:10]}"; print(f"  [{thread_name}] Starting Driverless for: {product_url_str}")
    ...
    try:
        ...
        if not driver: return None, product_name_guess
        ...
        try: 
            ...
            print(f"  [{thread_name}] Review container not found initially. Getting page source."); page_source_result = await driver.page_source; return page_source_result, product_name_guess
        ...
        if total_reviews_on_site is not None and total_reviews_on_site > 0:
            ...
            initial_review_elements_after_load = await driver.find_elements(By.CSS_SELECTOR, f"{REVIEW_CARDS_CONTAINER_SELECTOR} {INDIVIDUAL_REVIEW_CARD_SELECTOR}", timeout=SELENIUM_ELEMENT_FIND_TIMEOUT_S / 2)
            reviews_loaded_initially = len(initial_review_elements_after_load)
            if reviews_loaded_initially == 0 and total_reviews_on_site > 0: print(f"    [{thread_name}] WARNING: No reviews found initially in DOM, but site indicates {total_reviews_on_site}. Proceeding with clicks.")
            if reviews_loaded_initially >= total_reviews_on_site: print(f"    [{thread_name}] All {total_reviews_on_site} reviews ({reviews_loaded_initially} in DOM) seem to be loaded initially. No clicks needed."); num_clicks_needed = 0
            else:
                reviews_remaining_to_load = total_reviews_on_site - reviews_loaded_initially
                num_clicks_needed = max(0, (reviews_remaining_to_load + REVIEWS_BATCH_SIZE - 1) // REVIEWS_BATCH_SIZE)
            ...
            for click_num in range(num_clicks_needed):
                current_review_elements_pre_click = await driver.find_elements(By.CSS_SELECTOR, f"{REVIEW_CARDS_CONTAINER_SELECTOR} {INDIVIDUAL_REVIEW_CARD_SELECTOR}", timeout=0.2) 
                ...
                if current_review_count_dom >= total_reviews_on_site: print(f"    [{thread_name}] DOM reviews ({current_review_count_dom}) meet/exceed target ({total_reviews_on_site}). Stopping clicks early."); break
                
                # More pronounced adaptive sleeps
                if show_more_clicks > 60 and show_more_clicks % 7 == 0: 
                    extra_sleep = random.uniform(1.5, 3.0); print(f"      [{thread_name}] Adaptive sleep for {extra_sleep:.2f}s after {show_more_clicks} clicks."); await asyncio.sleep(extra_sleep)
                elif show_more_clicks > 30 and show_more_clicks % 5 == 0: 
                    extra_sleep = random.uniform(0.8, 1.8); print(f"      [{thread_name}] Adaptive sleep for {extra_sleep:.2f}s after {show_more_clicks} clicks."); await asyncio.sleep(extra_sleep)
                ...
                try:
                    ...
                    if show_more_clicks > 25: await asyncio.sleep(random.uniform(0.2, 0.5)) # Small pre-click pause
                    
                    clicked = await try_click_element(driver, element=show_more_button_el, thread_name=thread_name, always_js_click=True)
                    if not clicked: print(f"    [{thread_name}] try_click (forced JS) failed for 'Show More' on click {show_more_clicks + 1}. Breaking loop."); break 
                    
                    show_more_clicks += 1
                    reviews_loaded_successfully = await async_wait_until_reviews_load(driver, current_review_count_dom, f"{REVIEW_CARDS_CONTAINER_SELECTOR} {INDIVIDUAL_REVIEW_CARD_SELECTOR}", LOADING_SPINNER_SELECTOR, AFTER_SHOW_MORE_CLICK_LOADING_TIMEOUT_S, thread_name)
                    if not reviews_loaded_successfully: print(f"      [{thread_name}] Wait condition indicated no new reviews loaded after click #{show_more_clicks} (iter {click_num+1}). Breaking loop."); break 
                    if show_more_clicks > 0 and show_more_clicks % 7 == 0 : await handle_specific_overlay_popup(driver, thread_name)
                except TimeoutException: print(f"  [{thread_name}] 'Show more' button not found (before click {show_more_clicks +1} of {num_clicks_needed}). Assuming all loaded or error."); break
                except Exception as e_sm_loop: print(f"  [{thread_name}] Error in 'Show more' loop (click {show_more_clicks +1}): {type(e_sm_loop).__name__} - {e_sm_loop}"); break 
            final_reviews_in_dom = await driver.find_elements(By.CSS_SELECTOR, f"{REVIEW_CARDS_CONTAINER_SELECTOR} {INDIVIDUAL_REVIEW_CARD_SELECTOR}", timeout=0.2)
            ...
            # Fallback logic
            print(f"    [{thread_name}] Could not determine total review count. Falling back to max_show_more_attempts (heuristic) logic.")
            ...
            for attempt_loop in range(max_show_more_attempts):
                ...
                if show_more_clicks > 60 and show_more_clicks % 7 == 0:
                    extra_sleep = random.uniform(1.5, 3.0); print(f"      [{thread_name}][Fallback] Adaptive sleep for {extra_sleep:.2f}s after {show_more_clicks} clicks."); await asyncio.sleep(extra_sleep)
                elif show_more_clicks > 30 and show_more_clicks % 5 == 0:
                    extra_sleep = random.uniform(0.8, 1.8); print(f"      [{thread_name}][Fallback] Adaptive sleep for {extra_sleep:.2f}s after {show_more_clicks} clicks."); await asyncio.sleep(extra_sleep)
                try:
                    ...
                    clicked = await try_click_element(driver, element=show_more_button_el, thread_name=thread_name, always_js_click=True)
                    if not clicked: print(f"    [{thread_name}][Fallback] try_click failed for 'Show More' on attempt {show_more_clicks + 1}. Assuming end."); break 
                    show_more_clicks += 1
                    reviews_loaded_successfully = await async_wait_until_reviews_load(driver, initial_review_count_dom, f"{REVIEW_CARDS_CONTAINER_SELECTOR} {INDIVIDUAL_REVIEW_CARD_SELECTOR}", LOADING_SPINNER_SELECTOR, AFTER_SHOW_MORE_CLICK_LOADING_TIMEOUT_S, thread_name)
                    if not reviews_loaded_successfully: print(f"      [{thread_name}][Fallback] Wait condition indicated no new reviews loaded after click #{show_more_clicks}. Assuming end."); break 
                    if show_more_clicks > 0 and show_more_clicks % 7 == 0 : await handle_specific_overlay_popup(driver, thread_name)
                except TimeoutException: print(f"  [{thread_name}][Fallback] 'Show more' button not found (iter {attempt_loop+1}). Assuming all loaded."); break
                except Exception as e_sm_loop: print(f"  [{thread_name}][Fallback] Error in 'Show more' loop (iter {attempt_loop+1}): {type(e_sm_loop).__name__} - {e_sm_loop}"); break 
            print(f"    [{thread_name}][Fallback] Heuristic loop finished. Clicks: {show_more_clicks}.")
        print(f"  [{thread_name}] 'Show more' process completed. Total clicks: {show_more_clicks}. Retrieving page source."); page_source_result = await driver.page_source
        ...
        if driver:
            ...
            try: await driver.quit(); print(f"  [{thread_name}] Driver quit successfully.")
            except Exception as e_quit: print(f"  [{thread_name}] Error during driver.quit(): {type(e_quit).__name__} - {e_quit}")
    return page_source_result, product_name_guess

def parse_capterra_datetime_for_output(date_str: str) -> Tuple[Optional[str], Optional[datetime]]: # Unchanged
...
def _parse_individual_review_card_revised(review_card_soup: BeautifulSoup, thread_name: str) -> Optional[CapterraIndividualReview]: # Unchanged
    try:
...
def _parse_capterra_html_for_reviews(page_source: str, original_url_str: str, selenium_product_name_guess: str, thread_name: str) -> CapterraScrapeResultOutput: # Unchanged
    ...
    if overall_product_rating_el:
        ...
        if match:
            ...
            if match.group(2): review_count_from_display = int(match.group(2).replace(",", ""))
    if not review_count_from_display: 
        ...
        if review_count_display_el: 
            ...
            if count_match_of: review_count_from_display = int(count_match_of.group(1).replace(",", ""))
            else:
                count_match_showing_only = re.search(r"Showing\s+(\d+)\s+Reviews", text, re.IGNORECASE) 
                if count_match_showing_only: review_count_from_display = int(count_match_showing_only.group(1).replace(",", ""))
                else: 
                    count_match_total_only = re.search(r"^([\d,]+)\s+Reviews$", text.strip(), re.IGNORECASE)
                    if count_match_total_only: review_count_from_display = int(count_match_total_only.group(1).replace(",", ""))
...
async def scrape_capterra_async( product_url_str: str, start_date_filter_dt: Optional[datetime.date] = None, end_date_filter_dt: Optional[datetime.date] = None) -> Dict[str, Any]: # Unchanged
...
@app.post("/scrape-capterra", tags=["Capterra"]) # Unchanged
