...
if sys.platform == "win32" and sys.version_info >= (3, 8):
    try:
        asyncio.set_event_loop_policy(asyncio.WindowsSelectorEventLoopPolicy()) # Changed back from Proactor
...
try:
    import nodriver
    from nodriver import Config # Import Config
    print("INFO: nodriver library found.")
except ImportError as e:
    print(f"ERROR: {e}.")
    print("CRITICAL ERROR: nodriver library not found. Please install it: pip install nodriver")
    sys.exit(1)
...
app = FastAPI(
    title="Capterra Scraper API - Nodriver Loader",
    description="Nodriver loads all reviews, then BeautifulSoup parses.",
    version="3.0.2" 
)
...
# --- Constants for Capterra --- (Same as before)
NODRIVER_PAGE_TIMEOUT_S = 60
NODRIVER_ELEMENT_TIMEOUT_S = 20 
INITIAL_PAGE_LOAD_SLEEP_S = random.uniform(2.0, 3.5) 
AFTER_SHOW_MORE_CLICK_WAIT_S = 25 
...
POPUP_CLOSE_SELECTORS_CAPTERRA = ["#onetrust-accept-btn-handler",'div.sb.bkg-light.card.padding-medium i[data-modal-role="close-button"][class*="modal-close"]',"button[aria-label='Close' i]", "button[class*='modal__close' i]", "button[class*='CloseButton']","button[aria-label*='Dismiss' i]", "button[title*='Dismiss' i]","div[role='dialog'] button[class*='close' i]", "div[id^='ZN_'] button[aria-label='Close']", "button[id^='cookie-consent-accept']",]

async def attempt_to_close_popups_nodriver(page: nodriver.Tab, thread_name: str):
    closed_any = False
    specific_popup_selector = 'div.sb.bkg-light.card.padding-medium i[data-modal-role="close-button"]'
    try:
        specific_close_button = await page.find(specific_popup_selector, timeout=1.5) 
        if specific_close_button: 
            await specific_close_button.click(timeout=2)
            closed_any = True; await asyncio.sleep(0.7)
    except asyncio.TimeoutError: pass
    except Exception: pass 
    for sel in POPUP_CLOSE_SELECTORS_CAPTERRA:
        if sel == specific_popup_selector and closed_any: continue
        try:
            popup_btn = await page.find(sel, timeout=0.3)
            if popup_btn: 
                await popup_btn.click(timeout=1)
                closed_any = True; await asyncio.sleep(0.5)
                try: 
                    if not await page.find(sel, timeout=0.1): break 
                except asyncio.TimeoutError: break 
        except asyncio.TimeoutError: pass
        except Exception: pass 
    if closed_any: print(f"      [{thread_name}] Popup handling actions possibly taken."); await asyncio.sleep(random.uniform(0.5, 1.0))
...
def _parse_capterra_html_for_reviews(page_source: str, original_url_str: str, nodriver_product_name_guess: str, thread_name: str) -> CapterraScrapeResultOutput:
    soup = BeautifulSoup(page_source, DEFAULT_HTML_PARSER)
    parsed_reviews_list: List[CapterraIndividualReview] = []
    product_name_scraped = nodriver_product_name_guess; product_category_scraped = None
...
async def _load_all_capterra_reviews_nodriver_core( 
    product_url_str: str, company_slug: str, browser: nodriver.Browser # Expect browser instance
) -> Tuple[Optional[str], str]:
    thread_name = f"CapterraLoadNoDr-{company_slug[:10]}"
    page_content: Optional[str] = None
    tab: Optional[nodriver.Tab] = None
    product_name_guess = company_slug.replace("-"," ").title() 
    NODRIVER_INDIVIDUAL_REVIEW_CARD_LOCATOR = f"{REVIEW_CARDS_CONTAINER_SELECTOR} {INDIVIDUAL_REVIEW_CARD_SELECTOR}"

    try:
        if not browser or not browser.connected:
            print(f"  [{thread_name}] FATAL: Nodriver browser not connected or None.")
            return None, product_name_guess
            
        # Use the existing browser instance to get a new tab (or the first tab)
        if not browser.tabs: # Check if any tabs are open
            tab = await browser.new_tab(timeout=NODRIVER_PAGE_TIMEOUT_S)
        else: # Use the first available tab
            tab = browser.tabs[0] 
            if tab.closed: # If first tab is closed, open a new one
                 tab = await browser.new_tab(timeout=NODRIVER_PAGE_TIMEOUT_S)

        if not tab:
             print(f"  [{thread_name}] FATAL: Nodriver tab object is None after get() or new_tab() call!")
             return None, product_name_guess
            
        await tab.get(product_url_str, timeout=NODRIVER_PAGE_TIMEOUT_S) # Navigate the tab
            
        print(f"  [{thread_name}] Page loaded with Nodriver. Initial sleep for {INITIAL_PAGE_LOAD_SLEEP_S:.2f}s...")
        await asyncio.sleep(INITIAL_PAGE_LOAD_SLEEP_S)
        await attempt_to_close_popups_nodriver(tab, thread_name)
        
        try:
            h1_el = await tab.find(BS_PRODUCT_NAME_FALLBACK_H1_SELECTOR, timeout=3)
            if h1_el:
        ...
        print(f"    [{thread_name}] Nodriver phase - Product Name Context: {product_name_guess}")

        show_more_clicks = 0
        print(f"  [{thread_name}] Starting 'Show more reviews' loop with Nodriver...")
        
        try:
            await tab.find(REVIEW_CARDS_CONTAINER_SELECTOR, timeout=NODRIVER_ELEMENT_TIMEOUT_S / 2)
            print(f"    [{thread_name}] Review cards container found.")
        except asyncio.TimeoutError: 
            print(f"  [{thread_name}] Review container ('{REVIEW_CARDS_CONTAINER_SELECTOR}') not found by Nodriver.")
            page_content = await tab.content; return page_content, product_name_guess

        while True:
            initial_review_count_dom = len(await tab.select_all(NODRIVER_INDIVIDUAL_REVIEW_CARD_LOCATOR))
            try:
                show_more_button = await tab.find(SHOW_MORE_REVIEWS_BUTTON_SELECTOR, timeout=NODRIVER_ELEMENT_TIMEOUT_S / 2)
                if not show_more_button : 
                    print(f"    [{thread_name}] 'Show more' button not found by find(). Assuming end.")
                    break
                
                print(f"    [{thread_name}] Clicking 'Show More' (#{show_more_clicks + 1}). DOM reviews before: {initial_review_count_dom}.")
                await show_more_button.scroll_into_view()
                await asyncio.sleep(random.uniform(0.1, 0.3))
                
                await show_more_button.click(timeout=NODRIVER_ELEMENT_TIMEOUT_S / 3) 
                show_more_clicks += 1
                
                print(f"      [{thread_name}] Click initiated. Waiting for review count to increase (max {AFTER_SHOW_MORE_CLICK_WAIT_S}s)...")
                
                wait_start_time = time.monotonic()
                new_reviews_loaded_this_click = False
                while time.monotonic() - wait_start_time < AFTER_SHOW_MORE_CLICK_WAIT_S:
                    await asyncio.sleep(0.5) 
                    current_review_count = len(await tab.select_all(NODRIVER_INDIVIDUAL_REVIEW_CARD_LOCATOR))
                    if current_review_count > initial_review_count_dom:
                        print(f"      [{thread_name}] Review count increased to {current_review_count}.")
                        new_reviews_loaded_this_click = True
                        break
                
                if not new_reviews_loaded_this_click:
                    print(f"      [{thread_name}] Timeout waiting for review count increase. Before: {initial_review_count_dom}, After: initial_review_count_dom.")
                    break 
                
                await asyncio.sleep(random.uniform(0.1, 0.3))
                await attempt_to_close_popups_nodriver(tab, thread_name)
                
                current_review_count_after_wait = len(await tab.select_all(NODRIVER_INDIVIDUAL_REVIEW_CARD_LOCATOR))
                if current_review_count_after_wait == initial_review_count_dom and show_more_clicks > 0:
                    print(f"    [{thread_name}] DOM count stable after full wait cycle. Assuming end.")
                    break

            except asyncio.TimeoutError: 
                 print(f"  [{thread_name}] 'Show more reviews' button timed out (not found). Assuming all loaded.")
                 break
        ...
        print(f"  [{thread_name}] Error in Nodriver core loading ({product_url_str}): {type(e_core_load).__name__} - {e_core_load}")
        traceback.print_exc()
        if tab:
            try: page_content = await tab.content 
            except: pass
        return page_content, product_name_guess 
    finally:
        if tab and not tab.closed: # Close tab if it was successfully opened
            try: await tab.close()
            except Exception as e_tab_close: print(f"  [{thread_name}] Error closing tab: {e_tab_close}")


async def _load_all_capterra_reviews_nodriver_entry(
    ...
    try:
        config = Config()
        config.headless = False # Set to False for debugging, True for production
        config.sandbox = False # Often needed, especially in containers
        # config.browser_args = ['--lang=en-US,en'] # Example: set language
        
        print(f"  [Nodriver Entry] Attempting to launch Nodriver browser for {product_url_str}...")
        browser = nodriver.Browser(config=config)
        
        if not browser or not browser.connection:
            print(f"  [Nodriver Entry] Failed to initialize or connect Nodriver browser.")
            return None, product_name_guess
        print(f"  [Nodriver Entry] Nodriver browser launched and connected.")

        return await _load_all_capterra_reviews_nodriver_core(product_url_str, company_slug, browser)
    except Exception as e:
        print(f"  [Nodriver Entry] Error setting up Nodriver for {product_url_str}: {type(e).__name__} - {e}")
        traceback.print_exc()
        return None, product_name_guess
    finally:
        if browser:
            try: 
                print(f"  [Nodriver Entry] Attempting to close Nodriver browser for {product_url_str}...")
                await browser.close()
                print(f"  [Nodriver Entry] Nodriver browser closed for {product_url_str}.")
            except Exception as e_brw_close: 
                print(f"  [Nodriver Entry] Error closing nodriver browser for {product_url_str}: {e_brw_close}")
...
def scrape_capterra_sync_wrapper(
    ...
    page_source, product_name_nodriver_guess = asyncio.run(
        _load_all_capterra_reviews_nodriver_entry(product_url_str, company_slug)
    )
    if not page_source:
        return {"status": "error", "message": "Failed to load page with Nodriver.", 
                "data": None, "summary": {"product_url": product_url_str, "product_name_guess": product_name_nodriver_guess}}
    thread_name_parse = f"CapterraParse-{company_slug[:15]}"
    parsed_data_obj = _parse_capterra_html_for_reviews(page_source, product_url_str, product_name_nodriver_guess, thread_name_parse)
...
async def scrape_capterra_endpoint(request: ScrapeRequest = Body(...)) -> Dict[str, Dict[str, Any]]:
    ...
    max_workers = min(len(request.urls), (os.cpu_count() or 1), 2) 
    print(f"  Using {max_workers} worker threads for Nodriver tasks.")
...
#     async def main_test_nodriver():
#         test_url_google_pay = "https://www.capterra.com/p/212826/Google-Pay/reviews/" 
#         result_data = scrape_capterra_sync_wrapper(test_url_google_pay)
#         print(json.dumps(result_data, indent=2, default=str))
#     asyncio.run(main_test_nodriver())